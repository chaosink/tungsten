        ...
        const Vec3f &p2, const Vec3f &p3, const Vec3f &Ng)
{
    ...
    if (length0 == 0.0f || length1 == 0.0f || length2 == 0.0f || length3 == 0.0f)
        return 0.0f;
    Float Q =
          std::acos(clamp(n0.dot(n1)/(length0*length1), Float(-1.0f), Float(1.0f)))
        + std::acos(clamp(n1.dot(n2)/(length1*length2), Float(-1.0f), Float(1.0f)))
        + std::acos(clamp(n2.dot(n3)/(length2*length3), Float(-1.0f), Float(1.0f)))
        + std::acos(clamp(n3.dot(n0)/(length3*length0), Float(-1.0f), Float(1.0f)));

    return max(TWO_PI - std::abs(Q), Float(0.0f));
}
